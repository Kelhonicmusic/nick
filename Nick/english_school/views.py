...
def home_view(request):
    return render(request, 'english_school/base.html') 
...
def register_student(request):
    ...
    return render(request, 'english_school/register.html', {'form': form, 'role': 'Student'})
...
def student_dashboard(request):
    ...
    enrollments = Enrollment.objects.filter(student=student, payment_confirmed=True, expiry_date__gte=timezone.now())
    lessons = Lesson.objects.filter(student=student, completed=False)
    return render(request, 'english_school/student_dashboard.html', {'enrollments': enrollments, 'lessons': lessons})
